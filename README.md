...
- 🖱️ **마우스 드래그** — 사슬 끝을 잡고 잡아당기거나 흔들 수 있음
...
| 입력 | 동작 |
|---|---|
| **마우스 좌클릭 + 드래그** | 사슬 끝(또는 임의 Link)을 잡고 이동 |
| **마우스 떼기** | 사슬 해제, 관성으로 흔들림 |
| **R 키** | 사슬 + 상자 리셋 |
| **스페이스바** | 사슬에 강한 임펄스 가하기 (테스트용) |
...
2. **사슬 = 직사각형 Link** — 원형 Link 대비 시각적으로 "녹슨 사슬 고리" 느낌이 강하고, 회전 시 음영 변화가 자연스러움.
3. **density/friction 튜닝** — `density: 0.05` + `friction: 0.9` + `frictionAir: 0.01` 조합으로 묵직함 표현. 기본값은 너무 가벼움.
4. **마우스 Constraint 분리** — 사슬 끝을 잡았을 때만 잠그고, 그 외에는 자유 낙하. IK 체인 같은 자연스러운 곡선 유도.
5. **상자 = 정적/동적 혼합** — 바닥상자는 정적(벽), 무너질 나무상자는 동적 + `restitution: 0.1` (덜 튕김).
